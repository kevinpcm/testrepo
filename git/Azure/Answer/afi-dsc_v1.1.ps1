...
param (
    [parameter(Mandatory=$False)] [string] $InputFile  = "afi-csv_v1.1.csv"
      )
    ...
    foreach ($row in $csvData) {
        ...
        $ipName        = "$vmName"+"pip1"
            ...
            $uuPubIP		= $using:PubIP
                    ...
                    # NIC Creation with Public IP
                    Write-Output "creating NIC with Public IP: $uunicName..."
                    ...
                    # NIC Creation without Public IP
                    Write-Output "creating NIC without Public IP: $uunicName..."
                    $nic = New-AzureRmNetworkInterface -Name $uunicName -ResourceGroupName $uurgName -Location $uuLocation -SubnetId $subnet -PrivateIpAddress $uuprivIP    
                    }
...
function DSCTask
    ...
    $sPassLocal = ConvertTo-SecureString "Answer2830" -AsPlainText -force
    $sPassDom   = ConvertTo-SecureString "Answer2830" -AsPlainText -force
    $local      = New-Object System.Management.Automation.PsCredential -ArgumentList "localhost\install",$sPassLocal
    $domain     = New-Object System.Management.Automation.PsCredential -ArgumentList "install@go.local",$sPassDom
