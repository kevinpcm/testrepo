...
param (
    ...
    [parameter(Mandatory=$False)] [string] $InputFile  = "Kevin_qa__Add_Domain_Controllers.csv"
)
...
if ($csvData -ne $null) {
    ...
    foreach ($row in $csvData) {
        ...
        $ipName         = "$vmName"+"pip1"
            ...
            $nic = New-AzureRmNetworkInterface -Name $nicName -ResourceGroupName $rgName -Location $Location -SubnetId $vnet.Subnets[0].Id -PublicIpAddressId $pip.Id -PrivateIpAddress $privIP
