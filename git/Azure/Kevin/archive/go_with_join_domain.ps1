...
param (
    ...
    [parameter(Mandatory=$False)] [string] $InputFile       = "c:\scripts\go.csv",
...
if ($csvData -ne $null) {
    ...
    foreach ($row in $csvData) {
        ...
        $ipName         = "$vmName"+"pip1"
            ...
            $nic = New-AzureRmNetworkInterface -Name $nicName -ResourceGroupName $rgName -Location $Location -SubnetId $subnet -PublicIpAddressId $pip.Id -PrivateIpAddress $privIP -DnsServer $dns01
            ...
            Write-Output "`tJoining Domain: $DomainName"
            $templateFile = "azuredeploy.json"
            $parameterFile = "azuredeploy.parameters.json"
            New-AzureRmResourceGroupDeployment -ResourceGroupName $rgName -TemplateFile $templateFile -TemplateParameterFile $parameterFile -vmname $vmName -ouPath $OU -location $location
        }
