...
param (
    [parameter(Mandatory=$False)] [string] $InputFile  = "afi-csv.csv"
      )
...
function DSCTask
    ...
    $data       = import-csv .\afi-csv.csv 
        ...
        Copy-Item -Path .\afi-config-iis.ps1 -Destination $destination -Force -ToSession $TargetSess
            $ScriptBlock =
            {
        ...
        $vName      = $row.Name            
        $IP         = $row.PrivateIP
        $jobname    = "DSC_SQL" + "$vName"
        New-PSSession -ComputerName $IP -Credential $local -Verbose 
        $TargetSess = (Get-PSSession -ComputerName $IP -Credential $local  | where {$_.State -eq 'opened'})
        $destination= "c:\scripts\localhost.ps1"
        Copy-Item -Path .\afi-config-sql.ps1 -Destination $destination -Force -ToSession $TargetSess
            $ScriptBlock =
            {
            $domcred = $using:domain
            cd "C:\scripts"; 
