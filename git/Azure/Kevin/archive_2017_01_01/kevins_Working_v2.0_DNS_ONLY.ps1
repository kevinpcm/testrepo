...
param (
    [parameter(Mandatory=$False)] [string] $InputFile  = "god.csv"
      )
...
foreach ($row in $csvData)
    ...
    # Write-Output "Starting 5 Minute Sleep!"
...
function DSCTask
    ...
    $data       = import-csv .\god.csv | Where-Object {$_.Function -eq "DNS"} 
        ForEach ($row in $data) 
        {
		$vName     = $row.Name            
        $IP         = $row.PrivateIP
        ...
        Copy-Item -Path .\DSC_DNS_CONFIG_Version_1.1.ps1 -Destination $destination -Force -ToSession $TargetSess
            $ScriptBlock =
            {
