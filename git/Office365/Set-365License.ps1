...
if (!($SessionAvailable)) {
    function Connect-Office365 {
        ...
        $adminuser = "ZKB0399@conagrafoods.com"
...
function Set-ConAgraLicense {
    ...
    Begin {
        ...
        $BaseDisabled = 'Deskless', 'FLOW_O365_P2', 'POWERAPPS_O365_P2', 'TEAMS1', 'PROJECTWORKMANAGEMENT', 'SWAY', 'MCOSTANDARD'
        ...
        if ($basesku) {
            if ($user.licenses.accountskuid -match $SkuE3) {
                ForEach ($License in $user.licenses | Where {$_.AccountSkuID -eq $SkuE3}) {
                    $License.ServiceStatus | ForEach {
                        if ($_.ServicePlan.ServiceName -eq "Deskless" -or $_.ServicePlan.ServiceName -eq "FLOW_O365_P2" -or $_.ServicePlan.ServiceName -eq "POWERAPPS_O365_P2" -or $_.ServicePlan.ServiceName -eq "TEAMS1" -or $_.ServicePlan.ServiceName -eq "PROJECTWORKMANAGEMENT" -or $_.ServicePlan.ServiceName -eq "SWAY" -or $_.ServicePlan.ServiceName -eq "MCOSTANDARD") {
                            $DisabledOptions += $_.ServicePlan.ServiceName
                        }
                    }
                }
