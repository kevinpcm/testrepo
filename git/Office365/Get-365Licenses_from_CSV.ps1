...
$licensetype = Get-MsolAccountSku # | Where {$_.SkuPartNumber -eq "ENTERPRISEPACK"} # -or $_.SkuPartNumber -eq "EMS"} 
$csv = import-csv .\9457.csv
...
foreach ($license in $licensetype) {     
    ...
    foreach ($row in $($license.ServiceStatus)) { 
        ...
        # Build headerstring by looping through each option in a SKU(licensetype)
        # These SWITCHES ARE FOR OPTIONS
        ...
        # Add above Options that match a switch to the headerstring plus a comma
        $headerstring = ($headerstring + "," + $thisLicense) 
    } 
     
    Out-File -FilePath $LogFile -InputObject $headerstring -Encoding UTF8 -append 
     
    write-output ("Gathering users with the following subscription: " + $license.accountskuid) 
    ...
    foreach ($user in $users) {
        ...
        # Write-Output $userLicenses.count
        for ($i = 0; $i -lt $($userLicenses.count); $i++) {
            $userSkuId = $userLicenses[$i].AccountSkuId
            #write-output "$($user.DisplayName) , $userSkuId"

            if ($userSkuId -eq $skuid) {
                write-output ("Processing " + $user.displayname)
...
write-output ("Script Completed.  Results available in " + $LogFile)