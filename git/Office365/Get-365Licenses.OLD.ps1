...
$Sku = @{ 
    ...
    "POWERAPPS_INDIVIDUAL_USER" = "POWERAPPS_INDIVIDUAL_USER"
...
$licensetype = Get-MsolAccountSku |Where {$_.SkuPartNumber -eq "ENTERPRISEPACK"} 
...
# $skuid is being set to "cagrecipe:ENTERPRISEPACK"
$skuid = $license.accountskuid

# define $users variable - contains all msoluser for each member of CSV
$csv = import-csv C:\scripts\CAphaseTEST.csv # don't have to do each time ... place higher in code
$users = foreach ($user in $csv.userprincipalname) { 
    Get-MsolUser -UserPrincipalName $user 
}

# $User iterating each ROW in $users (Looping through each User in CSV)
foreach ($user in $users) {
    # $userLicenses = @()
    for ($j=0; $j -lt $($user.Licenses.accountskuid.count); $j++){
    if ($user.Licenses.accountskuid[$j] -eq 'cagrecipe:ENTERPRISEPACK'){
        $UserLicenses += $user.Licenses.accountskuid[$j]
        write-host "LICENSES: $UserLicenses"
        
    }
}    

write-host "COUNT: $UserLicenses.COUNT"
